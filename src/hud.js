...
export class HUD {
  constructor(uiRoot) {
    ...
    this.lapEl = el('div', 'hud-lap', this.tl, '<span class="lbl">圈数</span><span class="val">1</span><span class="of">/3</span>');
    ...
    this.driftPill = el('div', 'drift-pill', this.speedo, '漂移');
    this.placeEl = el('div', 'hud-place', this.br, '<span class="suf">第</span><span class="num">1</span><span class="suf">名</span>');
    this.placeNum = this.placeEl.querySelector('.num');

    // overlays
    this.countEl = el('div', 'hud-countdown', r);
    this.bannerEl = el('div', 'hud-banner', r);
    this.wrongEl = el('div', 'hud-wrongway', r, '<div class="ww-arrow"></div><div class="ww-text">逆行！</div>');
  ...
  _subscribe() {
    ...
    on('race:go', () => { this.showCount('出发！', 'go'); clearTimeout(this._cdT); this._cdT = setTimeout(() => this.countEl.classList.remove('show'), 1100); });
    on('race:finalLap', () => this.banner('最后一圈！', 'final'));
    on('race:lap', (d) => {
    ...
    on('race:finish', (d) => {
      if (d.kart && d.kart.isPlayer) {
        const p = d.place || 1;
        this.finishEl.innerHTML = `<div class="fin-title">冲线！</div><div class="fin-place" style="color:${PLACE_COLORS[p - 1] || '#fff'}">第${p}名</div>`;
  ...
  update(dt, { player, karts, race, itemSystem, time = 0 } = {}) {
    ...
    if (L.splits !== lt.length) {
      ...
      this.splitsEl.innerHTML = lt.map((t, i) => `<div class="split${t === best && lt.length > 1 ? ' best' : ''}"><span>第${i + 1}圈</span>${formatTime(t)}</div>`).join('');
    }
    ...
    if (L.dl !== dl) {
      ...
      this.driftPill.textContent = dl >= 3 ? '究极' : dl === 2 ? '超级' : dl === 1 ? '迷你' : '漂移';
    }
  ...
  showResults(results, { onRestart, onMenu, laps } = {}) {
    ...
    const rows = results.map((r, i) => {
      ...
      return `<div class="res-row${r.isPlayer ? ' me' : ''}" style="--d:${0.25 + i * 0.07}s">
        <div class="res-place" style="color:${pc}">第${r.place}名</div>
        <div class="res-portrait" style="--kc:${col}">${img ? `<img src="${img}" alt="">` : `<span>${(r.name || '?')[0]}</span>`}</div>
        <div class="res-name">${r.name}${r.isPlayer ? ' <em>（你）</em>' : ''}</div>
        <div class="res-best">${r.bestLap ? '最快圈 ' + formatTime(r.bestLap) : ''}</div>
        <div class="res-time">${r.estimated ? '~' : ''}${formatTime(r.time)}</div>
      </div>`;
    }).join('');
    const me = results.find((r) => r.isPlayer);
    const title = me ? (me.place === 1 ? '冠军！' : me.place <= 3 ? '登上领奖台！' : '比赛完成') : '比赛结果';
    R.innerHTML = `
      <div class="res-panel">
        <div class="res-title">${title}</div>
        <div class="res-sub">${laps || ''} 圈比赛 · 最终排名</div>
