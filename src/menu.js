...
const DIFF_LABEL = { easy: '50cc · 简单', normal: '100cc · 普通', hard: '150cc · 困难' };
const LAPS = [1, 3, 5];
const STAT_KEYS = [['speed', '极速', '极速'], ['accel', '加速', '加速'], ['handling', '操控', '操控'], ['weight', '重量', '重量']];
const HAT_LABEL = { cap: '棒球帽', crown: '皇冠', mushroom: '蘑菇', horns: '恐龙', shell: '龟壳', bow: '蝴蝶结' };
...
const CONTROLS_HTML = `
  <div class="ctl"><span class="kc">↑</span><span class="kc">W</span> 加速</div>
  <div class="ctl"><span class="kc">↓</span><span class="kc">S</span> 刹车 / 倒车</div>
  <div class="ctl"><span class="kc">←→</span><span class="kc">A D</span> 转向</div>
  <div class="ctl"><span class="kc wide">SPACE</span> 跳跃 / 漂移</div>
  <div class="ctl"><span class="kc">E</span><span class="kc">X</span><span class="kc wide">L-SHIFT</span> 使用道具</div>
  <div class="ctl"><span class="kc">C</span> 回头看</div>
  <div class="ctl"><span class="kc wide">ESC</span><span class="kc">P</span> 暂停</div>
  <div class="ctl"><span class="kc">M</span> 静音</div>`;
...
export class Menu {
  ...
  _buildSelect() {
    ...
    s.innerHTML = `
      <div class="sel-header"><div class="sel-title">选择你的车手</div><div class="sel-back">ESC · 返回</div></div>
      <div class="sel-body">
  ...
  _buildPause() {
    ...
    p.innerHTML = `
      <div class="pause-panel">
        ...
        <button class="btn" data-a="resume">继续游戏</button>
        <button class="btn" data-a="restart">重新开始</button>
        <button class="btn" data-a="quit">返回主菜单</button>
  ...
  _buildLoading() {
    this.loadingEl = el('div', 'screen loading-screen', this.uiRoot, '<div class="spinner"></div><div class="loading-text">加载中…</div>');
  }
  ...
  _refreshPreview() {
    ...
    this.pv.name.textContent = ch.name;
    this.pv.swatch.style.background = `linear-gradient(135deg, ${hex(ch.color)} 60%, ${hex(ch.accent)} 60%)`;
    this.pv.kartLbl.textContent = `${HAT_LABEL[ch.hat] || ch.hat} · 卡丁车`;
